...
const SignUp = ({ type = 'student' }) => {
  ...
  const checkEmailExists = async (emailToCheck) => {
    try {
      const response = await axios.get(`http://localhost:5001/api/students/all-students`);

      if (response.data.success) {
        setEmailExists(response.data.exists);
        if (!response.data.exists) {
          setHaveInvitationCode(false); // Reset institution code if email doesn't exist
        }
      }
    } catch (error) {
      ...
      <div className="flex flex-col">
        ...
        {emailExists && (
          <>
            ...
            <div
              onClick={() => setHaveInvitationCode(!haveInvitationCode)}
